...
def determine_best_models(values):
    ...
    models = list(filter(lambda name: not any(
        [ex_name in name for ex_name in excluded_models]), cate_keys))
    frameworks = ['T', 'S', 'X']
    selected_keys = []
...
def draw_figures(full_dict, x_values, x_ticks, axis_name, prefix_name, f_draw_all=True):
    ...
    for file_name, d in zip(('mean', 'median'), (d_mean, d_med)):
        ...
        if f_draw_all:
            ...
            all_models = list(filter(lambda name: not any(
                [sp_name in name for sp_name in spoiler_models]), cate_keys))
